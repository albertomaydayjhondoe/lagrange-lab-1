...
const Admin = () => {
  const { academyId } = useOutletContext<AcademyOutletContext>() ?? { academy: null, academyId: null };
  ...
  // Rol real del usuario en LA ACADEMIA ACTIVA (no un email hardcodeado).
  // owner/admin de esta academia -> acceso completo; el resto -> solo lectura.
  const { isAdmin: isAcademyAdmin, isOwner, loading: roleLoading } = useAcademyRole(academyId);
  const isAdmin = isOwner || isAcademyAdmin;

  // Verificar si es platform admin (para Global Backup - SPRINT 4 decisión de diseño)
  // Global Backup solo debe estar disponible para platform admins (tabla platform_admins)
  // Decisión: opción (a) - restrictiva por seguridad, requiere revisión humana para cambio
  useEffect(() => {
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const confirmImport = window.confirm(
        `¿Importar ${data.topology_nodes.length} nodos, ${data.topology_edges.length} tensiones y ${data.socratic_questions.length} preguntas EN ESTA ACADEMIA?\n\nEsto reemplazará los datos existentes de esta academia (no afecta a otras academias).`
      );
      ...
      if (!academyId) {
        throw new Error('No se pudo determinar la academia activa');
      }

      // Delete existing data — SOLO de la academia activa, nunca global.
      await Promise.all([
      ...
      // Insert new data — cada fila queda etiquetada con la academia activa.
      const { error: nodesError } = await supabase
        .from('topology_nodes')
        .insert(data.topology_nodes.map((n: TopologyNode) => ({ ...n, academy_id: academyId })));
      
      if (nodesError) throw nodesError;

      const { error: edgesError } = await supabase
        .from('topology_edges')
        .insert(data.topology_edges.map((e: TopologyEdge) => ({ ...e, academy_id: academyId })));
      
      if (edgesError) throw edgesError;

      const { error: questionsError } = await supabase
        .from('socratic_questions')
        .insert(data.socratic_questions.map((q: SocraticQuestion) => ({ ...q, academy_id: academyId })));
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {isAdmin && (
                  <>
                    ...
                    <DifferentialImport onRefresh={fetchData} disabled={dataLoading} academyId={academyId || ''} />
                  </>
                )}
                {/* 
                  SPRINT 4 - Decisión de diseño:
                  Backup ZIP es exclusho de platform admins (tabla platform_admins).
                  Este botón descarga TODOS los datos de la plataforma, no solo de la academia.
                  Por seguridad, solo los platform admins pueden acceder a esta función.
                */}
                {isPlatformAdmin && (
                  <Button
                    ...
                    title="Solo platform admins - descarga todos los datos de la plataforma"
                  >
                    {backupLoading ? (
                  ...
                  <>
                    <TabsContent value="rag">
                      ...
                      >
                        ...
                        <RAGSourcesEditor academyId={academyId} isAdmin={isAdmin} />
                      </motion.div>
                    </TabsContent>
