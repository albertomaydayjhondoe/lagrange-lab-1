...
function validateInput(body: unknown): {
  ...
  // Validate academyId (optional - defaults to genesis)
...
serve(async (req) => {
  ...
  try {
    ...
    const validation = await validateAcademyMembership(supabase, user.id, requestedAcademyId);
    ...
    const academyId = validation.academyId!;
    const axes = validation.axes || [];

    // Fetch data filtered by academy_id using shared context
    const [questionsRes, nodesRes] = await Promise.all([
      supabase.from("socratic_questions").select("*").eq("academy_id", academyId),
      supabase.from("topology_nodes").select("id, label, description, axis").eq("academy_id", academyId),
    ]);
